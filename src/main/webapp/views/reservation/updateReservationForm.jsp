...
<body>
	...
	<form action="/updateReservationAction" method="POST">
		<input type="hidden" id="reservationNum" name="reservationNum"
			value="${reservation.reservationNum}"> <input type="hidden"
			id="carCode" name="carCode" value="${reservation.carCode}">
		...
		<p>
			대여 일시 : <input type="datetime-local" id="start-datetime"
				name="start-datetime"
				value="<fmt:formatDate value="${reservation.borrowDate}" pattern="yyyy-MM-dd'T'HH:mm"/>" />
		</p>
		<p>
			반납 일시 : <input type="datetime-local" id="end-datetime"
				name="end-datetime"
				value="<fmt:formatDate value="${reservation.returnDate}" pattern="yyyy-MM-dd'T'HH:mm"/>" />
		</p>
